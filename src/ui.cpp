...
// wx3 is Unicode: conversion bridges for the C functions inherited from 2009
static inline bool ParseMoney(const wxString& str, int64& nRet)
{
    return ParseMoney(std::string(str.mb_str()).c_str(), nRet);
}
...
void AddPendingCustomEvent(wxEvtHandler* pevthandler, int nEventID, const T pbeginIn, const T pendIn)
{
    ...
    const char* pbegin = (pendIn != pbeginIn) ? (const char*)&pbeginIn[0] : NULL;
    const char* pend = pbegin + (pendIn - pbeginIn) * sizeof(pbeginIn[0]);
    wxCommandEvent event(nEventID);
    // wxString in wx3 is Unicode and does not carry raw bytes;
    // the serialized data travels in a buffer that the receiver frees
    event.SetClientData(new std::string(pbegin, pend));
...
{
    std::string* pstrData = (std::string*)event.GetClientData();
    if (pstrData == NULL)
        return CDataStream(SER_NETWORK);
    CDataStream ss(pstrData->data(), pstrData->data() + pstrData->size(), SER_NETWORK);
    delete pstrData;
    ((wxCommandEvent&)event).SetClientData(NULL);
    return ss;
}
...
{
    EndModal(true); // wx3: modal dialogs must EndModal, not Close()
    //Destroy();
}
...
{
    EndModal(false); // wx3: modal dialogs must EndModal, not Close()
}
...
CAboutDialog::CAboutDialog(wxWindow* parent) : CAboutDialogBase(parent)
{
    ...
    // (wx3 is natively Unicode; the wx2.8 UTF-8 workaround is no longer needed)
}
...
{
    // Called from worker threads: repaint only on the GUI thread
    if (wxTheApp)
...
CYourAddressDialog::CYourAddressDialog(wxWindow* parent, const string& strInitSelected) : CYourAddressDialogBase(parent)
{
    ...
    m_listCtrl->InsertColumn(1, "Bitflash Address", wxLIST_FORMAT_LEFT, 350);
    m_listCtrl->SetFocus();
...
void CYourAddressDialog::OnButtonRename(wxCommandEvent& event)
{
    ...
    CGetTextFromUserDialog dialog(this, "Rename Bitflash Address", "New Name", strName);
    if (!dialog.ShowModal())
        return;
...
{
    // Ask name
    CGetTextFromUserDialog dialog(this, "New Bitflash Address", "Name", "");
    if (!dialog.ShowModal())
        return;
...
class CMyApp: public wxApp
{
  public:
    ...
    // The MSYS2 release wxWidgets DLLs ship with asserts enabled. Log any
    // assertion to debug.log instead of showing a modal "Debug Alert" popup,
    // so the packaged build runs cleanly by double-click.
    virtual void OnAssertFailure(const wxChar* file, int line, const wxChar* func,
                                 const wxChar* cond, const wxChar* msg)
    {
        printf("wxAssert: %s(%d) %s: %s %s\n",
               (const char*)wxString(file).mb_str(), line,
...
bool CMyApp::OnInit()
{
    // The 2009 wxFormBuilder layout mixes wxEXPAND with alignment flags, which
    // modern wxWidgets flags as an assertion. Disable those sizer consistency
    // checks globally so no "Debug Alert" popup ever blocks the UI.
...
bool CMyApp::OnInit2()
{
    ...
    //
    // Parameters (parsed before the lock so /datadir and /port feed the mutex)
    //
    ...
    //
    // Limit to single instance per user/datadir
    // The mutex includes the datadir to allow multiple nodes on one machine.
    //
    const char* pszHomePath = getenv("HOMEPATH");
    wxString strMutexName = wxString("Bitflash.") + (pszHomePath ? pszHomePath : "default") + "." + strSetDataDir.c_str();
    ...
    if (psingleinstancechecker->IsAnotherRunning())
    {
        ...
        loop
        {
            ...
            HWND hwndPrev = FindWindow("wxWindowClassNR", "Bitflash");
            if (hwndPrev)
            {
    ...
    //
    {
        ...
        // Tests
        //
        if (argc >= 2 && wxString(argv[1]).CmpNoCase("/send") == 0)
        {
            int64 nValue = 1;
            if (argc >= 3)
...
void MainFrameRepaint()
{
    if (pframeMain)
    {
        ...
        // Called from worker threads: repaint only on the GUI thread
        wxTheApp->CallAfter([] { if (pframeMain) pframeMain->Refresh(); });
    }
}
